  ...
  const handleAddRestaurant = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({ name: '', address: '', phone: '', email: '', managerCode: '8888' });
        fetchRestaurants();
      }
    } catch (err) {
          ...
          {restaurants.map((shop) => (
            <div key={shop.id} className="premium-card rounded-[2.5rem] p-8 flex flex-col gap-6 group relative overflow-hidden">
               ...
               <div className="flex justify-between items-start">
                  <div className="w-16 h-16 rounded-2xl bg-white/[0.03] border border-white/5 flex items-center justify-center text-primary shadow-inner">
                     {shop.logoUrl ? (
                       <img src={shop.logoUrl} className="w-full h-full object-cover rounded-2xl" />
                     ) : (
               ...
               <div>
                  <h3 className="text-xl font-black text-white mb-1 group-hover:text-primary transition-colors">{shop.name}</h3>
               ...
               <div className="mt-auto pt-2 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-2">
                     <button className="flex items-center justify-center gap-2 py-3 rounded-xl bg-white/[0.03] border border-white/5 text-[10px] font-black uppercase tracking-widest text-gray-400 hover:bg-white/[0.05] hover:text-white transition-all">
                        <Settings className="w-3.5 h-3.5" />
                        Settings
                     </button>
                     <button className="flex items-center justify-center gap-2 py-3 rounded-xl bg-primary text-white text-[10px] font-black uppercase tracking-widest shadow-xl shadow-primary/10 hover:shadow-primary/30 transition-all">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 backdrop-blur-xl bg-black/60 animate-in fade-in duration-300">
          <div className="w-full max-w-xl bg-[#0b101a] border border-white/5 rounded-[3rem] p-10 shadow-2xl overflow-hidden relative">
             <div className="absolute top-0 right-0 p-8">
                <button onClick={() => setShowAddModal(false)} className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center text-gray-500 hover:text-white hover:bg-rose-500/20 transition-all">
                   <Plus className="w-6 h-6 rotate-45" />
                </button>
             </div>

             <div className="mb-10">
                <h2 className="text-2xl font-black text-white uppercase tracking-tight mb-2">Onboard New Shop</h2>
                <p className="text-gray-500 font-bold uppercase tracking-[0.2em] text-[10px]">Create a new instance for your POS network</p>
             </div>
             ...
             <form onSubmit={handleAddRestaurant} className="space-y-6">
                ...
                <div className="pt-6">
                   <button 
                     type="submit"
                     className="w-full bg-primary text-white py-5 rounded-2xl font-black text-xs uppercase tracking-widest shadow-2xl shadow-primary/20 hover:shadow-primary/40 transition-all active:scale-[0.98]"
                   >
                     Complete Onboarding
                   </button>
                </div>
